...
double Pi_Function::f(double p,double q,double w){
  ...
  if(w!=0 && p*q*q*q/4/m/m<w*w*1e-6){
    //return p*m/2/pi/pi/q/(std::exp((p*p/2/m-mu)/T)+1)*32*p*q*q*q/4/m/m/(w*w+(q*q*q*q+4*p*p*q*q)/4/m/m);
    return 8*q*q/pi/pi/w/w*p*p/(std::exp((p*p/2/m-mu)/T)+1)*m/2/4/m/m;
  }
...
Pi_Function calc_pi(std::string label){
...
Pi_Function test_pi(std::string label){
  // read config from pi.in
...
if(argc==2){
  ...
  Pi_Function pf=calc_pi(slabel);

  H5::H5File file(name1,H5F_ACC_TRUNC);
